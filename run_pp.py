...
def evaluate_agent(env, plot=False, save=False):
    episode_reward = list()
    ...
    for _ in range(num_episodes):
        ...
        if plot and save:
            plot_array(np.array(moving_average(speed, window_size=9)), linewidth=1, filename=f'PPspeed{_}.png', ylabel='Speed(m/s)')
            plot_array(np.array(moving_average(dist_from_center, window_size=9)), linewidth=1, filename=f'PPdist_from_center{_}.png', ylabel='Deviation(m)')
            plot_array(np.array(moving_average(lat_jerk, window_size=9)), linewidth=1, filename=f'PPlat_jerk{_}.png', ylabel='Lateral jerk(m/s^3)')
            plot_array(np.array(moving_average(lkd, window_size=9)), linewidth=1, filename=f'PPlookahead{_}.png', ylabel='Look-ahead(m)')
            plot_array(np.array(moving_average(accel, window_size=9)), linewidth=1, filename=f'PPlat_acceleration{_}.png', ylabel='Lateral Acceleration(m/s^2)')
            plot_array(np.array(moving_average(rotation_v, window_size=9)), linewidth=1, filename=f'PProtationV{_}.png', ylabel='w(rad/s)')
            plot_path(np.array(path), np.array(vpath), filename=f'PPpath{_}.png')
        ...
        save_list_to_csv(speed, f"csvfile/speed{_}_pp{lookahead}.csv")
        save_list_to_csv(dist_from_center, f"csvfile/deviation{_}_pp{lookahead}.csv")
        save_list_to_csv(accel, f"csvfile/accel{_}_pp{lookahead}.csv")
        save_list_to_csv(lat_jerk, f"csvfile/latjerk{_}_pp{lookahead}.csv")
        save_list_to_csv(vpath, f"csvfile/vpath{_}_pp{lookahead}.csv")
        save_list_to_csv(lkd, f"csvfile/final_lkd{_}_pp{lookahead}.csv")
    data_analysis={
    ...
    df.to_csv(f't3_right_performance_each_episode_pp{lookahead}.csv', index=False)
...
try:
    ...
    # 创建环境
    env = CarEnv(sync=True, train=False, town=TOWN, pathfile=PATHFILE,determined_speed=PIDController(.5, .25, .1, 0.05, L_MIN_SPEED), pp=True)
