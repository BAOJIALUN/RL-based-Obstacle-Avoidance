...
class PPOPolicyNetContinuous(torch.nn.Module):
    ...
    def forward(self, x):
        x = F.relu(self.fc1(x))
        mu = 2.0 * torch.tanh(self.fc_mu(x))   # steer 输出限制在 [-2, 2]（可根据任务改）
        std = F.softplus(self.fc_std(x)) + 1e-5  # 避免 std 为 0
        return mu, std
...
class PPOContinuous:
    ...
    def __init__(self, state_dim, hidden_dim, action_dim, actor_lr, critic_lr,
        ...
        self.actor = PPOPolicyNetContinuous(state_dim, hidden_dim, action_dim).to(device)
    ...
    def take_action(self, state):
        ...
        action = dist.sample()
        return action.clamp(-1.0, 1.0).cpu().numpy()[0]  # steer 范围 [-1, 1]，throttle [0,1] 请在 env 中限制    
    ...
    def update(self, transition_dict):
        ...
        dones = torch.tensor(transition_dict['dones'], dtype=torch.float32).view(-1, 1).to(self.device)        # Optional reward normalization
        td_target = rewards + self.gamma * self.critic(next_states) * (1 - dones)
        td_delta = td_target - self.critic(states)
        advantage = compute_advantage(self.gamma, self.lmbda, td_delta.cpu()).to(self.device).view(-1, 1)        
